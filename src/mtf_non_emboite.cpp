...
int main(int argc, char* argv[]){
   ...
   std::vector<double> kappa(nc+1),mu(nc+1);
    ...
    // Noeuds du maillage
    Geometry node(("../meshes/maillage_non_emboite_"+nc_str+".msh").c_str());
    ...
    const std::vector<R3>& N=NormalTo(Omega[0]);
    std::vector<int> nb_elt;
    for (int i=0;i<NbDom;i++){
        nb_elt.push_back(NbElt(Omega[i]));
    }
    double ratio = 1.5;
    ...
    for(int I=0; I<NbDom; I++){
        ...
        for(int j=0; j<ne; j++){
            ...
            // A(nN[I]+dofI[j], nD[I]+dofI[j]) += (-0.5)*MassP1(e);
            // A(nD[I]+dofI[j], nN[I]+dofI[j]) += (-0.5)*MassP1(e);
        }
    }

    Mass.Export(("../output/matrices/Mass_non_emboite_"+nc_str+"_"+type_str+".txt").c_str());
    ...
    N2 interface[NbInt];
    for(int j=0; j<NbInt; j++){
        interface[j][0]=0; interface[j][1]=j+1;
    }
    ...
    P.Export(("../output/matrices/Prec_non_emboite_"+nc_str+"_"+type_str+".txt").c_str());
    ...
    file.open("../output/matrices/A_non_emboite_"+nc_str+"_"+type_str+".txt");
    ...
    file.open("../output/matrices/M_non_emboite_"+nc_str+"_"+type_str+".txt");
    for(int j=0; j<nb_dof_tot; j++){
        for(int k=0; k<nb_dof_tot; k++){
            Cplx z = M(j,k);
            file << z.real() << " ";
    ...
    file.open("../output/matrices/P_non_emboite_"+nc_str+"_"+type_str+".txt");
    for(int j=0; j<nb_dof_tot; j++){
        for(int k=0; k<nb_dof_tot; k++){
            Cplx z = P(j,k);
            file << z.real() << " ";
    ...
    file.open("../output/matrices/f_non_emboite_"+nc_str+"_"+type_str+".txt");
    ...
    file.open("../output/matrices/uex_non_emboite_"+nc_str+"_"+type_str+".txt");
    for(int j=0; j<f.size(); j++){
        Cplx z = vex[j];
        file << z.real() << " ";
        file << z.imag() << " ";
    ...
    file.close();
    // ofstream out,out_b;
    // out.open("../output/matrices/test_"+nc_str+"_"+type_str+".txt");
    // out_b.open("../output/matrices/test_b_"+nc_str+"_"+type_str+".txt");
    // for (int i=0;i<nb_dof_tot;i++){
    //     for (int j=0;j<nb_dof_tot;j++){
    //         out<<std::real(A(i,j));
    //         out_b<<std::real(M(i,j));
    //         if (std::imag(A(i,j))<0){
    //             out<<std::imag(A(i,j))<<"i\t";
    //         }
    //         else if (std::imag(A(i,j))==0){
    //             out<<"+"<<0<<"i\t";
    //         }
    //         else{
    //             out<<"+"<<std::imag(A(i,j))<<"i\t";
