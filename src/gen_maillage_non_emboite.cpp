...
int main(int argc, char* argv[]){
    ...
    double ratio = 1;
    int nb_node = 0;
    vector<double> center(nc),theta(nc);
    vector<int> nv(nc);

    for(int j=0; j<nc; j++){
        center[j]   = (j!=0) ? 2*ratio+ratio+center[j-1]: 0 ;
        nv[j]    = floor(ratio*2*pi/h);
        theta[j] = 2*pi/double(nv[j]);
        nb_node += nv[j];
    }

    ofstream meshfile;
    meshfile.open(("../meshes/maillage_non_emboite_"+std::to_string(nc)+".msh").c_str());
    meshfile << "$MeshFormat\n2.2 0 8\n$EndMeshFormat\n$Nodes\n";
